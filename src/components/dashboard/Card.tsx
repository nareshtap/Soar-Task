...
const Card: React.FC<CardProps> = ({ balance, cardHolder, validThru, cardNumber, isWhite }) => {
    return (
        <div className={` ${isWhite ? 'bg-white' : 'bg-[linear-gradient(107.38deg,#5B5A6F_2.61%,#000000_101.2%)]'} border border-[rgba(223,234,242,1)] ${isWhite ? 'text-black' : 'text-white'} rounded-3xl shadow-lg w-full h-full`}>
            <div className="flex flex-col gap-8">

                <div className='flex justify-between items-center pt-6 px-6'>
                    <div className="flex flex-col">
                        <span className={`'text-xs ${isWhite ? 'text-[#718EBF]' : 'text-white'}'`}>Balance:</span>
                        <h5 className={`text-[20px] font-semibold leading-[24px] ${isWhite ? 'text-black' : 'text-white'}`}>${balance}</h5>
                    </div>
                ...
                <div className='flex w-[calc(100%-100px)] gap-24 px-6'>
                    <div className="flex flex-col">
                        <span className={`'text-xs ${isWhite ? 'text-black' : 'text-white'}'`}>Card Holder:</span>
                        <h5 className={`'text-[20px] font-semibold leading-[24px] ${isWhite ? 'text-black' : 'text-white'}'`}>{cardHolder}</h5>
                    </div>
                    <div className="flex flex-col">
                        <span className={`'text-xs ${isWhite ? 'text-black' : 'text-white'}'`}>Valid Thru:</span>
                        <h5 className={`'text-[20px] font-semibold leading-[24px] ${isWhite ? 'text-black' : 'text-white'}'`}>{validThru}</h5>
                    </div>
                </div>
